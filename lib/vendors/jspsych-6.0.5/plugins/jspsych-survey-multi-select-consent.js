...
jsPsych.plugins['survey-multi-select-consent'] = (function() {
  var plugin = {};

  plugin.info = {
    name: 'survey-multi-select-consent',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var cssstr = ".jspsych-survey-multi-select-question { margin-top: 2em; margin-bottom: 2em; text-align: left; font-size: 17px;}"+
      ".jspsych-survey-multi-select-text span.required {color: darkred;}"+
      ".jspsych-survey-multi-select-horizontal .jspsych-survey-multi-select-text {  text-align: center; font-size: 17px;}"+
    ...
    trial_form.addEventListener('submit', function(event) {
      ...
      for(var index=0; index<matches.length; index++){
        ...
        if(val.length != 3){ has_response.push(false); } else { has_response.push(true); }
      }
      ...
      if(trial.required && has_response.includes(false)) {
        ...
        display_element.querySelector(".fail-message").innerHTML = '<span style="color: red; font-size; 14px" class="required">Please check all boxes</span>';
      } else {
        // save data
        var trial_data = {
          "rt": response_time,
