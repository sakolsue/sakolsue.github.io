...
jsPsych.plugins['survey-multi-select'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var cssstr = ".jspsych-survey-multi-select-question { margin-top: 2em; margin-bottom: 2em; text-align: left; font-size: 15px;}"+
      ".jspsych-survey-multi-select-text span.required {color: darkred;}"+
      ".jspsych-survey-multi-select-horizontal .jspsych-survey-multi-select-text {  text-align: center; font-size: 15px;}"+
    ...
    trial_form.addEventListener('submit', function(event) {
      ...
      if(trial.required && has_response.includes(false)) {
        ...
        display_element.querySelector(".fail-message").innerHTML = '<span style="color: red; font-size; 14px" class="required">Please answer the question.</span>';
      } else {
        // save data
        var trial_data = {
          "rt": response_time,
